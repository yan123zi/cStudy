...
const char fileName[] = "data/item.dat";
FILE *itemFile = NULL;
HashTable *ItemHashTable = NULL;
...
int closeFile()
{
    int ret = fclose(itemFile);
    if (ret != 0)
    {
        printf("closeFile fclose failed");
        return -1;
    }
...
int addItemToFile(const struct Item *item, const int offset)
{
    const int ret = fseek(itemFile, offset, SEEK_SET);
    if (ret != 0)
    {
